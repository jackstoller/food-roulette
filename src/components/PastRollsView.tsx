        ...
        try {
            ...
            const message =
                err instanceof Error
                    ? err.message
                    : 'Unknown error while loading rolls';
            setError(message);
        } finally {
            setIsLoading(false);
        }
            ...
            {!isLoading && userId && error && (
                <div className='bg-red-50 border border-red-200 text-red-700 rounded-xl p-4 mb-4'>
                    <p className='font-semibold mb-2'>
                        Could not load your roll history.
                    </p>
            ...
            {!isLoading && userId && !error && rolls.length === 0 && (
                <div className='bg-gray-50 border border-gray-100 text-center rounded-2xl p-6'>
                    ...
                    <p className='text-sm text-gray-500'>
                        Start rolling to build your delicious history!
                    </p>
                </div>
            )}
            ...
            {!isLoading && userId && !error && rolls.length > 0 && (
                <div className='space-y-3'>
                    {rolls.map((roll) => (
                        <div
                            key={roll.id}
                            className='bg-linear-to-r from-orange-50 to-red-50 p-4 rounded-xl border border-orange-100 hover:shadow-md transition'
                        >
                            <div className='flex justify-between items-start mb-2'>
                                <div>
                                    ...
                                    <p className='text-sm text-gray-600'>
                                        {roll.cuisine} · ⭐{' '}
                                        {roll.rating.toFixed(1)}
                                    </p>
                                </div>
                            </div>
                            <p className='text-xs text-gray-500 font-medium mb-1'>
                                {formatDateLabel(roll.rolledAt)}
                            </p>
                            <p className='text-sm text-gray-600'>
                                {roll.address}
                            </p>
