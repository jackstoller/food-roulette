...
function filterPlaces(places: Place[], params?: PlacesQueryParams): Place[] {
    ...
    return places.filter((place) => {
        ...
        if (params.cuisine) {
            ...
            if (
                cuisines.length > 0 &&
                !cuisines.includes('Any') &&
                !cuisines.includes(place.cuisine)
            ) {
                return false;
            }
        }
        ...
        // Filter by price
        if (params.price) {
            const prices = Array.isArray(params.price)
                ? params.price
                : [params.price];
...
export const api = {
            ...
            if (params) {
                ...
                if (params.price) {
                    if (Array.isArray(params.price)) {
                        params.price.forEach((p) =>
                            queryParams.append('price', p.toString())
                        );
                    } else {
    ...
    async getRandomPlace(params?: PlacesQueryParams): Promise<Place> {
        ...
        try {
            ...
            if (!result.success || !result.data) {
                throw new Error(
                    result.message || 'Failed to fetch random place'
                );
            }
