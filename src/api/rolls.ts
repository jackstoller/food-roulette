...
export const rollsApi = {
    async fetchHistory(
        ...
        const response = await fetch(
            `${API_BASE_URL}/rolls?${query.toString()}`
        );

        if (!response.ok) {
            throw new Error(
                `Failed to load roll history (status ${response.status})`
            );
        }
        ...
        if (!payload.success || !payload.data) {
            throw new Error(
                payload.message || 'Roll history response was malformed'
            );
        }
    ...
    async recordRoll(params: {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to persist roll (status ${response.status})`
            );
        }
        ...
        if (!payload.success || !payload.data) {
            throw new Error(
                payload.message || 'Roll persistence response was malformed'
            );
        }
