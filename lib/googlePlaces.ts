...
function convertOldGooglePlaceToPlace(
    googlePlace: OldGooglePlace,
    index: number
): Place {
    return {
        id: index,
...
export async function searchNearbyPlaces(
    ...
    try {
        ...
        let places: Place[] = (data.results || []).map(
            (place: OldGooglePlace, index: number) =>
                convertOldGooglePlaceToPlace(place, index)
        );

        // Log successful API call with photo info
        const placesWithPhotos = places.filter(
            (p) => !p.image.includes('unsplash')
        ).length;
        console.log(
            `✅ Loaded ${places.length} restaurants from Google Places API (${placesWithPhotos} with real photos)`
        );

        // Apply client-side filters
        if (priceLevels && priceLevels.length > 0) {
            places = places.filter((place) =>
                priceLevels.includes(place.price)
            );
        }
...
function filterMockPlaces(params: SearchNearbyParams): Place[] {
    ...
    if (
        params.cuisineTypes &&
        params.cuisineTypes.length > 0 &&
        !params.cuisineTypes.includes('Any')
    ) {
        places = places.filter((place) =>
            params.cuisineTypes!.includes(place.cuisine)
        );
    }
    ...
    if (params.priceLevels && params.priceLevels.length > 0) {
        places = places.filter((place) =>
            params.priceLevels!.includes(place.price)
        );
    }
