...
export function normalizeFiltersSnapshot(filters?: Partial<RollFiltersSnapshot>): RollFiltersSnapshot {
  return {
    cuisine: filters?.cuisine && filters.cuisine.length > 0 ? filters.cuisine : ['Any'],
    price: filters?.price && filters.price.length > 0 ? filters.price : [1, 2, 3],
...
function mapRowToRecord(row: any): RollRecord {
  ...
  if (row.filters) {
    try {
      ...
      console.warn('Failed to parse roll filters JSON, using defaults', error);
    }
  }
...
export function recordRoll({ userId, place, filters }: RecordRollParams): RollRecord {
  const db = getDb();
...
export function buildFiltersSnapshot(params: SnapshotParams): RollFiltersSnapshot {
